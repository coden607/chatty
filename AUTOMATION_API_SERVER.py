...
# In-memory cache for weekly brief to avoid redundant AI calls
_brief_cache = {
    "body": None,
    "expires_at": 0
}
...
async def weekly_brief():
    ...
    if _brief_cache["body"] and now < _brief_cache["expires_at"]:
    ...
    async with _brief_cache_lock:
        ...
        body = {
            ...
            "events": events[:6],
            "summary": summary,
            "cached_at": datetime.fromtimestamp(now).isoformat()
        }

        # Cache the result for 120 seconds
        _brief_cache["body"] = body
        _brief_cache["expires_at"] = now + 120
